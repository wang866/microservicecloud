...
### 2 、构建公共子模块 microservicecloud-api
...
### 3、构建服务提供者 microservicecloud-provider-dept-8001
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <artifactId>microservicecloud-provider-dept-8001/服务提供者8001</artifactId>
...
### 4、构建服务消费者 microservicecloud-consumer-dept-80
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <artifactId>部门微服务消费者</artifactId>
...
- 新建microservicecloud-euraka-7001工程，导入相关依赖
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <artifactId>Eureka注册中心-7001</artifactId>
    <dependencies>
        <!--eureka-server 服务端-->
        <dependency>
            <groupId>org.springframework.cloud</groupId>
...
- 修改客户端访问类80的controller，将地址改为微服务名称**MICROSERVICECLOUD-DEPT**

  ```java
  /**
   * 注册再EurekaServer中的微服务名称
   */
  private static final String REST_URL_PREFIX="http://MICROSERVVICECLOUD-DEPT";
  ```
...
参考microservicecloud-consumer-dept-80模块，新建microservicecloud-consumer-dept-feign模块
...
- 修改microservicecloud-api
  ...
  - pom文件修改
    ...
    添加Feign支持依赖，与microservicecloud-consumer-dept-feign一致
...
- 在microservicecloud-consumer-dept-feign修改Controller，修改基于上述配置的接口
 ...
 */
@EnableFeignClients(basePackages = "com.wzq.springcloud")
@ComponentScan("com.wzq.springcloud")
...
利用Ribbon维护了MICROSERVICECLOUD-DEPT的服务列表，并且通过轮询实现了客户端的负载均衡。而与Ribbon不同的是，==通过Feign只需要定义服务绑定接口且以声明式的方法==，优雅的而简单的实现了服务调用。

## 4、小结

Feign通过接口的方法调用Rest服务（之前是Ribbon+RestTemplate）

该请求发送给Eureka服务器（http://MICROSERVICECLOUD-DEPT/depe/list）通过Feign直接找到服务接口，由于再进行服务调用时候融合了Ribbon技术，所以也支持负载均衡
...
### 1、参考microservicecloud-provider-dept-8001新建，microservicecloud-provider-dept-hystrix-8001模块
...
- 修改DeptController
  ...
  ```java
	...
	public Dept get(@PathVariable("id") Long id) {
	    ...
	    if (null == dept) {
	        throw new RuntimeException("该" + id + "没有部门信息");
	    }
   ...
   public Dept processHystrix_Get(Long id) {
        return new Dept().setDeptno(id).setDname("该id没有对应的部门信息---@HystrixCommand")
...
- microservicecloud-consumer-dept-feign修改yml
...
- 构建模块项目microservicecloud-consumer-hystrix-dashboard
...
- 新建microservicecloud-zuul-gateway-9527模块
...
- 不启用路由：<http://localhost:8001/dept/list>
- 启用路由：<http://myzuul.com:9527/MICROSERVICECLOUD-DEPT/dept/list>
...
- 修改yml，增加如下配置
  ...
  配置后<http://myzuul.com:9527/MICROSERVICECLOUD-DEPT/dept/list>

  可变为http://myzuul.com:9527/mydept/dept/list
...
### 1、在github上新建microservicecloud-config的repository

https://github.com/wangzq95928/microservicecloud-config.git

### 2、在本地硬盘中新建git仓库并clone

git clone https://github.com/wangzq95928/microservicecloud-config.git
...
### 4、新建module，microservicecloud-config-3344
  ...
  spring:
    ...
    cloud:
      config:
        server:
          git:
            # uri: git@github.com:wangzq95928/microservicecloud-config.git
            uri: https://github.com/wangzq95928/microservicecloud-config.git
  ```
...
### 1、在本地git仓库中新建一个microservicecloud-config-client.yml
...
- yml文件
  ...
  - bootstrap.xml
    ...
    spring:
	  cloud:
	    config:
	      name: microservicecloud-config-client #需要从github上服务的资源名称，注意没有yml后缀
